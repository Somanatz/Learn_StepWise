...
export default function LessonPage() {
  ...
  const handleGenerateSummary = async () => {
    ...
    setAiSummary(null);
    try {
        // Here you would have an endpoint that calls the Genkit flow and returns the summary
        const result = await api.post<{ processed_output: string }>('/ai/notes/summarize/', { lesson_id: lesson.id });
        setAiSummary(result.processed_output);
    } catch(err:any) {
  ...
  const handleTranslate = async () => {
    ...
    try {
        // Endpoint that calls Genkit translation flow
        const result = await api.post<{ translated_content: string }>('/ai/translate/', { lesson_id: lesson.id, language: targetLanguage });
