...
export default function CompleteStudentProfilePage() {
  ...
  useEffect(() => {
    const fetchClassesForSchool = async (schoolId: string) => {
        if (!schoolId) {
            setClasses([]);
            form.resetField("enrolled_class_id");
            return;
        }
        try {
  ...
  const onSubmit = async (data: StudentProfileFormValues) => {
    ...
    Object.keys(data).forEach(key => {
        ...
        if (K === 'profile_picture') return; // Handled separately
        if (data[K] !== undefined && data[K] !== null) {
             if (typeof data[K] === 'boolean') {
                 formData.append(key, String(data[K]));
             } else if (data[K] !== '') { // Ensure empty strings are not sent if not intended
    ...
    try {
      ...
      setCurrentUser(prev => prev ? { ...prev, ...updatedUser, student_profile: updatedUser.student_profile || prev.student_profile } : null);
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField control={form.control} name="full_name" render={({ field }) => (
               ...
               <FormField control={form.control} name="nickname" render={({ field }) => (
                <FormItem><FormLabel>Nickname (Optional)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )} />
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <FormField control={form.control} name="date_of_birth" render={({ field }) => (
                    <FormItem><FormLabel><CalendarClock className="inline mr-1 h-4 w-4"/>Date of Birth (Optional)</FormLabel><FormControl><Input type="date" {...field} /></FormControl><FormMessage /></FormItem>
                )} />
                <FormField control={form.control} name="place_of_birth" render={({ field }) => (
                    <FormItem><FormLabel>Place of Birth (Optional)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                )} />
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
               ...
               <FormField control={form.control} name="parent_occupation" render={({ field }) => (
                <FormItem><FormLabel>Parent's Occupation (Optional)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )} />
              ...
              <div className="grid md:grid-cols-2 gap-4">
                 ...
                 <FormField control={form.control} name="parent_mobile_for_linking" render={({ field }) => (
                    <FormItem><FormLabel>Parent's Mobile for Linking (Optional)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                 )} />
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <FormField control={form.control} name="blood_group" render={({ field }) => (
                    <FormItem><FormLabel><Droplets className="inline mr-1 h-4 w-4"/>Blood Group (Optional)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                )} />
              </div>

              <FormField control={form.control} name="hobbies" render={({ field }) => (
                <FormItem><FormLabel><Gamepad2 className="inline mr-1 h-4 w-4"/>Hobbies (Optional)</FormLabel><FormControl><Textarea {...field} /></FormControl><FormMessage /></FormItem>
              )} />
              <FormField control={form.control} name="favorite_sports" render={({ field }) => (
                <FormItem><FormLabel><HeartPulse className="inline mr-1 h-4 w-4"/>Favorite Sports (Optional)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )} />

              <div className="grid md:grid-cols-2 gap-4">
                <FormField control={form.control} name="needs_assistant_teacher" render={({ field }) => (
                    <FormItem className="flex items-center space-x-2 pt-6">
                        <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} id="needs_assistant_teacher" /></FormControl>
                ...
                <FormField control={form.control} name="interested_in_gardening_farming" render={({ field }) => (
                    <FormItem className="flex items-center space-x-2 pt-6">
                        <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} id="interested_in_gardening_farming"/></FormControl>
